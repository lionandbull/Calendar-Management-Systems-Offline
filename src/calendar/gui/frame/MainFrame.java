...
//	public static MainFrame mainFrame = new MainFrame();
	...
	public JToolBar buildToolBar() {  
		...
		toolBar.add(buildButton("Remove calendar")); 
...
//		panel.setBorder(BorderFactory.createTitledBorder(""));  
	...
	private class PanelAction implements ActionListener {  
		...
		if (btnName == "New calendar") {
			if (myCalendars.size() == 0 && flag == false) {
				new CMABoard();
				flag = true;
			}
			else {
			...
			if (myCalendars.size() == 0) {
				...
				String s = (String)JOptionPane.showInputDialog(
				                    panel,
				                    "Please choose a calendar to load.",
				                    "Load calendar",
				...
				if ((s != null) && (s.length() > 0)) {
					...
					for (MyCalendar mc : myCalendars) {
						if (mc.getName().equals(s)) {
							CMABoard.load(myCalendars.get(i));
						}
						i++;
					}
				    return;
		...
		else if (btnName == "Remove calendar") {
			if (myCalendars.size() == 0) {
				JOptionPane.showMessageDialog(panel,
						"There is no available calendar.",
						"Error",
					    JOptionPane.ERROR_MESSAGE);
		}
		else {
			...
			String s = (String)JOptionPane.showInputDialog(
			                    panel,
			                    "Please choose a calendar to remove.",
			                    "Remove calendar",
			                    JOptionPane.PLAIN_MESSAGE,
			                    null,
			                    possibilities,
